...
export class TerminarCompraComponent implements OnInit {
  ...
  public async revisarYTerminar(stepper) {
    ...
    if (!this.clienteModel.telefono) {
      return alert("Falta escribir el telefono del cliente");
    }
    if (!this.clienteModel.tarjeta) {
      return alert("Falta escribir la tarjeta del cliente");
    }
